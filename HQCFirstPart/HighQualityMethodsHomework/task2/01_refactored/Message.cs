...
class Message
{
    ...
    static void Main()
    {
        ...
        FindFirstDigitsInString(firstLine, firstNumber);
        FindSecondDigitsInString(secondLine, secondNumber);

        BigInteger first = BigInteger.Parse(firstNumber.ToString());
        BigInteger second = BigInteger.Parse(secondNumber.ToString());

        BigInteger result = FindCorrectOperator(operation, first, second);
        string curentResult = result.ToString();

        StringBuilder print = new StringBuilder();
        print = FindNumeralSystemDigits(curentResult, print);

        Console.WriteLine(print);
    }

    private static StringBuilder FindNumeralSystemDigits(string curentResult, StringBuilder print)
    {
        for (int i = 0; i < curentResult.Length; i++)
        {
            ...
            {
                throw new ArgumentException("The digit was not found!");
            }
        }
    ...
    private static BigInteger FindCorrectOperator(string operation, BigInteger first, BigInteger second)
    {
        BigInteger result = 0;
        if (operation == "-")
        {
    ...
    private static void FindSecondDigitsInString(string secondLine, StringBuilder secondNumber)
    {
    ...
    private static void FindFirstDigitsInString(string firstLine, StringBuilder firstNumber)
    {
